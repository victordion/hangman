...
bool letter_select_v1(char *str, char &letter, std::vector<char> selected, int word_number);
...
char machine_two(char * sentence, std::vector<char> selected, char &letter, char * guess, std::vector<std::string>& guessed_sentence)
{
	...
	//if(sentence_confab_v2(0,word_number)&&((total_char/unknown_char)>=2))
	if(sentence_confab_v1(0)&&((total_char/unknown_char)>=2))
	{
		//std::cout<<"Passed Sentence: "<<sentence<<std::endl;
	...
	else
	{
		//std::cout<<"Passed Sentence: "<<sentence<<std::endl;
		//std::cout<<"Confab Sentence: "<<recall_sentence<<std::endl;
		/*if(letter_select_v1(recall_sentence, letter, selected, word_number))
		{
			free_input_sentence(0);
			return 'N';
		}
...
bool sentence_confab_v2(int sid, int &word_number)
{
	...
	for(word=0;word<20;word++)
	{
		...
		if(word==0)
		{
			if((confidence[word]==0)&&(confidence[word+1]==0))
				return false;
		}
		else if(word==19)
		{
		...
		{
			if((confidence[word]==0)&&((confidence[word-1]==0)||(confidence[word+1]==0)))
			{
				return false;
			}
		}
