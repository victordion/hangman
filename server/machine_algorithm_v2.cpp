...
#include <cmath>
#include "recall.h"
#define CONFIDENCE_V1 0.15
#define CONFIDENCE_V2 0.25
...
extern int sentence_main(int,std::vector<std::vector<float> > &);
...
bool sentence_confab_v1(int);
bool sentence_confab_v2(int, int&);
void output_word();
...
char machine_two(char * sentence, std::vector<char> selected, char &letter, char * guess, std::vector<std::string>& guessed_sentence)
{
	...
	//if(sentence_confab_v1(0)&&((total_char/unknown_char)>=2))				//Syed's Algorithm v1
	{
		...
		{
			if(recall_sentence[i]==' ')
				continue;
			total_conf_char++;
		}
		//If number of characters in confabulated sentence is not equal to passed sentence
		if(total_conf_char!=total_char)
		{
			// Uncomment for Syed's Letter Confab
			/*if(letter_select_v1(recall_sentence, letter, selected))
				{
...
int collect_wconf_results(std::string str1, int word_count)
{
	...
	for(i=0;i<26;i++)
	{
		if(letter_select_candidate[i] != -1)
		{
			...
			{
				//letter_select_frequency[i] += letter_select_candidate[i]/(double)total_words_num; Wei's Letter
				letter_select_frequency[i] += letter_select_candidate[i]; //Syed's Letter v2
			}
			letter_select_candidate[i] = 0;
		}
	}
...
bool sentence_confab_v1(int sid)
{
	...
	for(word=0;word<20;word++)
	{
		max=0;
		max_1=0;
		for(word_candidate=0;word_candidate<excitationLvl.at(word).size();word_candidate++)
		{
			if(excitationLvl[word][word_candidate]!=0)
			{
				if(excitationLvl[word][word_candidate]>max)
					max = excitationLvl[word][word_candidate];
				else if(excitationLvl[word][word_candidate]>max_1)
					max_1 = excitationLvl[word][word_candidate];
				//printf("Candidate ID: %d \n Target ID: %d\n Excitation Level: %f \n",word,word_candidate,excitationLvl[word][word_candidate]);
			}
		}
		//If the different between max and 2nd max value is less than 20%, return false
		if(max!=0)
		{
			confidence = fmin(1,(max-max_1)/max_1);
			//printf("Max value: %f   2nd Max value: %f\n",max,max_1);
			if(confidence<=CONFIDENCE_V1)
			{
				//printf("Dont Confabulate \n");
				return false;
			}
		}
	}
...
bool sentence_confab_v2(int sid, int &word_number)
{
	...
	for(word=0;word<20;word++)
	{
		max[word]=0;
		max_1[word]=0;
		//std::cout<<"Number of candidates for Word "<<word<<" are "<<excitationLvl.at(word).size()<<std::endl;
		for(word_candidate=0;word_candidate<excitationLvl.at(word).size();word_candidate++)
		{
			if(excitationLvl[word][word_candidate]!=0)
			{
				if(excitationLvl[word][word_candidate]>max[word])
					max[word] = excitationLvl[word][word_candidate];
				else if(excitationLvl[word][word_candidate]>max_1[word])
					max_1[word] = excitationLvl[word][word_candidate];
				//printf("Word Number: %d \n Target ID: %d\n Excitation Level: %f \n",word,word_candidate,excitationLvl[word][word_candidate]);
			}
		}
		//std::cout<<"Word No: "<<word<<" Highest excitation: "<<max[word]<< " 2nd highest excitation: "<<max_1[word]<<std::endl;
		//max = 0 means word already guesssed
		if(max[word]!=0)
		{
	...
	for(word=0;word<20;word++)
	{
		...
		{
			if((confidence[word]<=CONFIDENCE_V2)&&((confidence[word-1]<=CONFIDENCE_V2)||(confidence[word+1]<=CONFIDENCE_V2)))
			{
				return false;
			}
		}
